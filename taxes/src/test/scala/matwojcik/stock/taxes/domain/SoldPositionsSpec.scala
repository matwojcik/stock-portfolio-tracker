...
class SoldPositionsSpec extends AnyFeatureSpec with Matchers with GivenWhenThen {
  ...
  Feature("finding sold positions") {
    ...
    Scenario("multiple buys, multiple taken") {
      ...
      Then("First transaction should be fully used, and the second only partially with proportionally used provision")
      soldPositions should contain only Income.SoldPosition(
        sell,
        NonEmptyList.of(buyTransaction, secondBuy.copy(quantity = Quantity(10), provision = Money(0.33, EUR)))
      )
    }
    ...
    Scenario("multiple stocks") {
      ...
      soldPositions should contain only (Income
        .SoldPosition(sellTransaction, NonEmptyList.of(buyTransaction)), Income.SoldPosition(sellStock2, NonEmptyList.of(buyStock2)))
    }
    ...
    Scenario("multiple sells in the same year") {
      ...
      soldPositions should contain only (
        Income.SoldPosition(firstSell, NonEmptyList.of(firstBuy.copy(quantity = Quantity(10), provision = Money(0.33, EUR)))),
        Income.SoldPosition(
          secondSell,
          NonEmptyList.of(
            firstBuy.copy(quantity = Quantity(20), provision = Money(0.67, EUR)),
            secondBuy.copy(quantity = Quantity(5), provision = Money(0.25, EUR))
          )
        ),
        Income.SoldPosition(thirdSell, NonEmptyList.of(secondBuy.copy(quantity = Quantity(15), provision = Money(0.75, EUR)))),
      )
    }
    ...
    Scenario("sell in previous years") {
      ...
      soldPositions should contain only (Income.SoldPosition(
        ...
        NonEmptyList.of(
          firstBuy.copy(quantity = Quantity(20), provision = Money(0.67, EUR)),
          secondBuy.copy(quantity = Quantity(10), provision = Money(0.5, EUR))
        )
      ))
    }
